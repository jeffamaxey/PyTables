            ...
            for count, col in enumerate(data):
                ...
                if the_type == 'str':
                    ...
                    for i in data:
                        if len(i) > the_max:
                            the_max = len(i)
                    make_col(row_type, f'col_{str(count)}', col[0], the_max)
                elif the_type:
                    make_col(row_type, f'col_{str(count)}', col[0], 0)
                else:
...
class Hdf_dict(dict):
    def __init__(self, hdf_file, hdf_dict=None, stack=None):
        ...
        self.stack = stack
        self.hdf_dict = hdf_dict if stack else self.get_cache()
    ...
    def get_cache(self):
        ...
        print('reading')
        begin = clock()
        total = [i['col_0'].decode() for i in table.iterrows()]
        total = ''.join(total)
                    ...
                    try:
                        ...
                        for row in table:
                            ...
                            if cur_row == num_rows:
                                ...
                                cur_column = []
                            cur_column.append(row[f'col_{str(col_num)}'])
                            cur_row += 1
                        total_columns.append(cur_column)
