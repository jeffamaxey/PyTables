...
class PyTables_DB(DB):
    ...
    def __init__(self, nrows, rng, userandom, datadir,
        ...
        self.filename = f"pro-{self.filename}"
        self.filename += f'-O{optlevel}'
        self.filename += f'-{kind}'
        if docompress:
...
#                        _testmode=True)
    ...
    def do_query(self, con, column, base, inkernel):
        ...
        condition = (
            "((inf2<=col) & (col<sup2)) | ((inf1<col2) & (col2<sup1))"
            + " & ((col1+3.1*col2+col3*col4) > 3)"
        )
...
#             results = table.read_coordinates(coords)
            ...
            results = [
                ...
                if inf2 <= r['col4'] < sup2
                or inf1 < r['col2'] < sup1
                and (r['col1'] + 3.1 * r['col2'] + r['col3'] * r['col4']) > 3
            ]
